...
#'     \itemize{
#'         \item "default": use the \code{re2} package
#'         (\url{https://github.com/girishji/re2}) for simple searches and the
...
#'         \item "re2": use \code{re2} for all searches.
#'     }
...
explore <- function(corpus_object,
  ...
  if ("package:corporaexplorer" %in% search() == FALSE) {
    stop(
      "Load 'corporaexplorer' by running 'library(corporaexplorer)' before launching app.",
      call. = FALSE
    )
  }
