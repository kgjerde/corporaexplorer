---
title: "How to use corporaexplorer"
output:
    github_document
---
...
This document presents **corporaexplorer**'s two main functions:
...
See the [reference section](reference/index.html) for full details
and all available options.

## 1. Prepare data for the Shiny apps

The `prepare_data()` function returns a "corporaexplorerobject" that can be explored in the package's Shiny app.
...
* `text_column`. Default is `Text`. The column in `dataset` containing the texts to be explored.
...
Start the app by running the `explore()` function with a
"corporaexplorerobject" created by `prepare_data()` as argument.
Run the following in the R console to see documentation for the `explore()` function.
...
* **Term(s) to chart and highlight**: Search terms separated by newline (enter). Using colour fillings, the corpus map plot will indicate days/documents where these terms are present. The terms will also be highlighted in all documents. Current maximum: six search terms.
* **Highlight additional terms**: Terms included here (one on each line) will be highlighted in all documents. An arbitrary number of terms can be added.
...
* **Date range by year or by date**: Filters the corpus (the first and last date included) by time span. (Only for date-based corpora.)
* **Case sensitive (slower)**: Check this box to distinguish between lower and upper case in the search. Unchecked, `war` will find both "war" and "War"; if checked, `war` will only find "war".
* **Corpus map height**: Change plot height by adjusting the bar and clicking the "Apply" button.
* **Left column %**: Higher % means wider corpus map box and more narrow document view box. Can also be adjusted by clicking and dragging between the boxes.
...
* **Search button**: Only when search button is clicked will input from the sidebar (with the exception  of the "Adjust plot size" input) be handled by the app. The search button can also be triggered by the keyboard combination cmd+enter (ctrl+enter on Windows) from within any text box.
...
For more about regex syntax and the regex flavours available, see the section about regex engines [below](#2d-advanced-detail-regular-expression-engines).
...
1. `default`: use the [`re2`](https://github.com/girishji/re2) package for simple searches and the [`stringr`](https://github.com/tidyverse/stringr) package for complex regexes (details below).
This is the recommended option.
2. use `stringr` for all searches.
3. use `re2`for all searches.  

`re2` is very fast but has a more limited feature set than `stringr`, especially
in handling non-ASCII text, including word boundary detection.
With the `default` option, the `re2` engine is run when no special regex characters are used;
otherwise `stringr` is used. This option should fit most use cases.

Please consult the documentation for [`re2`](https://github.com/girishji/re2) and
...
Advanced users can set `optional_info` to `TRUE` via the `search_options` argument in `explore()` (i.e. `search_options = list(optional_info = TRUE)`):
