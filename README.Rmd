...
**corporaexplorer** is an R package for dynamic exploration of corpora/text
collections through a `Shiny` app  graphical user interface.
...
Thus, the ambition is that the package should be useful for anyone with a rudimentary knowledge of R --
...
The package includes a tiny 'corporaexplorerobject': `corporaexplorer::test_data`, which consists of 10 tiny documents and can be used for testing the basic functionality of the package.

A more typical corpus can be downloaded for testing [here](https://ndownloader.figshare.com/files/14626970)
-- this is a 'corporaexplorerobject' based on a document collection scraped from the Russian governmental website kremlin.ru (more info [here](https://figshare.com/s/820c105c12755d853801)).
The equivalent material in Russian can be downloaded [here](https://figshare.com/s/be0d2cfa195e43160fcb).<sup>[2](#myfootnote2)</sup>
...
and returns a corporaexplorer object that can be explored in the package's two Shiny apps. The default arguments should work fine for most use cases.
...
Start the app by running the `run_corpus_explorer()` function with a corporaexplorer object created by `prepare_data()` as argument. Run the following in the R console to see documentation for the `run_corpus_explorer()` function.
...
Clicking on a "document tile" produces two things. First, the full text of the document with search terms highlighted. Second, above the text a tile chart consisting of n tiles where each tile represents a 1/n part of the document, and where the colour in a tile indicates whether and how many times the search term is found in that part of the document. Clicking on a tile scrolls the document to the corresponding part of the document.<sup>[1](#myfootnote1)</sup>
...
n handling non-ASCII text, including word boundary detection.
...
Start the app by running the `run_document_extractor()` function with a corporaexplorer object created by `prepare_data()` as argument. Run the following in the R console to see documentation for the `run_document_extractor()` function.
...
<a name="myfootnote1">*1</a>. Using the [`jquery.scrollTo`](https://github.com/flesler/jquery.scrollTo) plugin.*  
<a name="myfootnote2">*2</a>. All this material is licensed under Creative Commons Attribution 4.0 International (CC BY 4.0).*
