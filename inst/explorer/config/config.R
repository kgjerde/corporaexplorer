...
if (is.null(loaded_data)) {
  loaded_data <-
    corporaexplorer::prepare_data(
      "testthat has a 'dry run' of sorts, resulting in weird behaviour.",
...
if (!is.null(ui_options$corpus_map_column_width)) {
  if (is.numeric(ui_options$corpus_map_column_width)) {
    if (
      ui_options$corpus_map_column_width >= 30 &
        ui_options$corpus_map_column_width <= 70
    ) {
      CORPUS_MAP_COLUMN_WIDTH_PCT <- as.integer(
        ui_options$corpus_map_column_width
      )
    }
  }
}
...
if (!is.null(plot_options$colours)) {
  MAIN_COLOURS <- create_colours_from_input(
    plot_options$colours,
    MAIN_COLOURS
  )
}
