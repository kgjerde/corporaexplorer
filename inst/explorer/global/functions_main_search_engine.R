...
contains_whitespace <- function(pattern) {
  return(
    length(stringr::str_split(pattern, "\\s|\\\\s", simplify = TRUE)) != 1
  )
}
...
contains_other_regex_special_characters <- function(pattern) {
  stringr::str_detect(pattern, "[\\[\\]\\(\\)\\{\\}]") |
    stringr::str_detect(pattern, "\\\\[^BbWwSs]")
}
...
#' @return Logical.
can_use_matrix <-
  function(pattern, subset_search, case_sensitive, custom_column) {
    if (
      case_sensitive == TRUE |
        NO_MATRIX == TRUE |
        !is.na(custom_column)
...
count_matrix <- function(pattern, matriks, df, ordvektor) {
  ...
  ord_indekser <- which(regex_detect(ordvektor, pattern))
...
count_df <- function(pattern, df, case_sensitive, custom_column) {
  ...
  treff_count_matrix <-
    tibble::tibble(regex_count(
      text_column, # korpus_365$Text
