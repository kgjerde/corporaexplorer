...
#'
#' @return Adjusted df.
create_distance_coordinates_dok <- function(
  test1,
  linjer,
  row_distance = 0.15,
  ...
  if (linjer > 1) {
    ...
    test1 <- test1 %>%
      ...
      dplyr::mutate(
        x_min = x_min + (position_in_row * horizontal_distance),
  ...
  test1$Year_numeric <- as.numeric(factor(
    test1$Year_,
    levels = unique(test1$Year_)
  ))
  test1$y_min <- test1$y_min +
    (year_distance * test1$Year_numeric - min(test1$Year_numeric))
  test1$y_max <- test1$y_max +
    (year_distance * test1$Year_numeric - min(test1$Year_numeric))
  return(test1)
}
