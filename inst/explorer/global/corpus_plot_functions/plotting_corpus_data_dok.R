...
plotting_corpus_data_dok <-
  function(test1, y_text, legend_df, linjer) {
    a <- ggplot2::ggplot()
    ...
    rect_tib_vertikal$x_mid <- (rect_tib_vertikal$x +
      rect_tib_vertikal$xend) /
      2
    rect_tib_vertikal$y_mid <- (rect_tib_vertikal$y +
      rect_tib_vertikal$yend) /
      2
    ...
    if (length(unique(test1$Year_)) > 1) {
      ...
      coordinates_for_dividing_lines_btwn_groups <-
        (test1$y_min[coordinates_for_dividing_lines_btwn_groups] +
          test1$y_max[
            coordinates_for_dividing_lines_btwn_groups - 1
          ]) /
        2
