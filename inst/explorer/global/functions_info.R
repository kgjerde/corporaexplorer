create_df_for_info <- function(session_variables, search_arguments, modus) {
  if (modus == "data_dok") {
  ...
  reconstruct_full_terms <- function(term, optional1, optional2) {
    ...
    optional1[!is.na(optional1)] <- paste0(
      "--",
      optional1[!is.na(optional1)]
    )
    optional1[is.na(optional1)] <- ""

    optional2[!is.na(optional2)] <- paste0(
      "--",
...
show_corpus_info_table <- function(
  search_arguments,
  session_variables,
  start_df_list
) {
  ...
  df <- tidyr::gather(start_df) %>%
    ...
    dplyr::mutate(
      "Found in % of documents" = Documents / nrow(start_df) * 100
    ) %>%
...
show_corpus_info_text <- function(
  search_arguments,
  session_variables,
  original_data_dok,
  start_df_list
...
corpus_is_filtered <-
  function(search_arguments, session_variables, original_data_dok) {
    return(
      search_arguments$subset_search |
        nrow(session_variables$data_dok) != nrow(original_data_dok) |
        sum(session_variables$data_365$Invisible_fake_date) !=
          sum(loaded_data$original_data$data_365$Invisible_fake_date)
    )
  }

text_about_filtered_corpus <-
  function(session_variables, start_df_list) {
    start_df <- start_df_list$start_df
  ...
  if (info_terms_exist(search_arguments, info_mode)) {
    ...
    if (length(unique(fig_tib$Year_)) == 1 | DATE_BASED_CORPUS == FALSE) {
      info_plot <-
        info_plot +
        ggplot2::geom_col(
          ggplot2::aes(x = Year_, y = Count, fill = Term),
          position = "dodge"
        ) +
      ...
      info_plot <-
        ...
        ggplot2::scale_color_manual(values = correct_colors)
    }
    ...
    info_plot <- info_plot +
      ...
      ggplot2::scale_y_continuous(
        breaks = pretty(fig_tib$Count, n = 6, min.n = 1)
      ) +
