...
compute_navigable_days <- function() {
  ...
  valid_mask <- data$Invisible_fake_date == FALSE &
    is.na(data$Day_without_docs)
...
select_corpus_row <- function(min_rad) {
  ...
  if (plot_mode$mode == "data_365") {
    if (
      session_variables[[plot_mode$mode]]$Invisible_fake_date[min_rad] ==
        FALSE &
        is.na(session_variables[[plot_mode$mode]]$Day_without_docs[
          min_rad
        ])
    ) {
      source(
        "./server/3_event_corpus_map_click/UI_element_control_data_365.R",
        local = TRUE
      )
      source(
        "./server/3_event_corpus_map_click/preparing_day_corpus.R",
        local = TRUE
      )
      source(
        "./server/3_event_corpus_map_click/title_list.R",
        local = TRUE
      )
      source(
        "./server/3_event_corpus_map_click/rendering_day_corpus_map.R",
        local = TRUE
      )
      source(
        "./server/3_event_corpus_map_click/js_auto_scroll.R",
        local = TRUE
      )
      update_nav_arrows()
    }
    ...
    source(
      "./server/3_event_corpus_map_click/UI_element_control_data_dok.R",
      local = TRUE
    )
    source(
      "./server/3_event_corpus_map_click/display_document_text.R",
      local = TRUE
    )
    source(
      "./server/3_event_corpus_map_click/document_info_tab_data_dok.R",
      local = TRUE
    )
    source(
      "./server/3_event_corpus_map_click/document_visualisation.R",
      local = TRUE
    )
...
find_adjacent_row <- function(current, direction) {
  if (plot_mode$mode == "data_365") {
    ...
    if (
      next_idx >= 1 &&
        next_idx <= length(session_variables$navigable_days)
    ) {
...
update_nav_arrows <- function() {
  ...
  if (plot_mode$mode == "data_365") {
    ...
    if (is.null(current)) {
      ...
      if (can_navigate(-1)) {
        shinyjs::enable("nav_prev_day")
      } else {
        shinyjs::disable("nav_prev_day")
      }
      if (can_navigate(1)) {
        shinyjs::enable("nav_next_day")
      } else {
        shinyjs::disable("nav_next_day")
      }
    ...
    if (is.null(current)) {
      ...
      if (can_navigate(-1)) {
        shinyjs::enable("nav_prev_doc")
      } else {
        shinyjs::disable("nav_prev_doc")
      }
      if (can_navigate(1)) {
        shinyjs::enable("nav_next_doc")
      } else {
        shinyjs::disable("nav_next_doc")
      }
      # Update position indicator
